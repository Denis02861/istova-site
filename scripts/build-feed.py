#!/usr/bin/env python3
"""
ЗАПАСНОЙ генератор фида по 9 программам сайта. НЕ основной.

Основной фид Истовы — public/offers.yml (35 предложений, сделан 17.08.2026 руками,
включает «Теплоту» и сертификаты, подключён в кабинете Яндекса). Этот скрипт
собирал дубль feed.yml, дубль удалён 18.09.2026. Оставлен на случай, если
понадобится быстро пересобрать фид только по программам.
