...
const steps = [
  ...
  { n: "02", t: "Арома-выбор", d: "На ресепшене подбираете аромат, который будет сопровождать весь ритуал.", img: "/services/01-nastroyka.jpg" },
  { n: "03", t: "Программа", d: "Выбранный ритуал. От 70 до 150 минут вашего времени.", img: "/services/05-massage.jpg" },
  ...
  return (
    <section id="ritual" className="py-24 bg-sand relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <ol className="max-w-5xl mx-auto space-y-16 md:space-y-20">
          {steps.map((step, i) => {
            const withImg = !!step.img;
            const imgLeft = i % 2 === 0;
            return (
              <BlurFade key={step.n} delay={0.05} yOffset={30}>
                <li className={"relative flex flex-col " + (withImg ? "md:grid md:grid-cols-2 md:gap-10 md:items-center" : "md:flex-row md:gap-10 md:items-start")}>
                  {withImg && (
                    <div className={imgLeft ? "md:order-1" : "md:order-2"}>
                      <CardTilt maxTilt={6} scale={1.02}>
                        <div className="relative aspect-[4/5] rounded-sm overflow-hidden shadow-[0_20px_60px_-20px_rgba(116,68,54,0.4)] border border-brand/10">
                          <img src={step.img} alt={step.t} className="w-full h-full object-cover" loading="lazy" />
                        </div>
                      </CardTilt>
                    </div>
                  )}
                  <div className={"flex gap-6 md:gap-8 " + (withImg ? (imgLeft ? "md:order-2" : "md:order-1") : "")}>
