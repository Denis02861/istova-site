...
const items = [
  { src: "/gallery/head-spa/jade.jpg",   title: "Аква-дуга",           desc: "Ритуал под тёплой водой, японская техника" },
  { src: "/gallery/head-spa/wooden.jpg", title: "Деревянные скребки",  desc: "Массаж кожи головы + оливковое дерево как символ" },
  { src: "/gallery/head-spa/aurora.jpg", title: "Массаж лица",         desc: "Ручная работа мастера в свете aurora" },
];
      ...
      <div className="container mx-auto px-6 max-w-6xl relative z-10">
        ...
        <BlurFade delay={0.15} yOffset={16}>
          <p className="text-center text-brand-dark/70 mb-16 max-w-xl mx-auto leading-relaxed">
            Восточная техника ухода за головой и лицом. То, за чем возвращаются.
          </p>
        </BlurFade>
          ...
          {items.map((it, i) => (
            <BlurFade key={i} delay={0.05 + i * 0.1} yOffset={30}>
              <div className="relative aspect-[3/4] rounded-sm overflow-hidden shadow-[0_25px_70px_-25px_rgba(116,68,54,0.4)] border border-brand/10 bg-brand/5 group">
                {/* Ken Burns — постоянный медленный zoom+pan */}
                <img
                  src={it.src}
                  alt={it.title}
                  width={720}
                ...
                />
                {/* Тёмный градиент снизу для читаемости caption */}
                <div className="absolute inset-0 bg-gradient-to-t from-black/75 via-black/10 to-transparent pointer-events-none" />
                {/* Подпись — статичная */}
                <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8 text-sand">
                  <h3 className="font-display text-2xl md:text-3xl mb-2 tracking-tight">{it.title}</h3>
                  <p className="text-sm text-sand/90 leading-relaxed">{it.desc}</p>
                </div>
              </div>
            </BlurFade>
          ))}
