...
export default function Header() {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-[background-color,box-shadow,border-color,height,max-height,opacity] duration-300 ease-out ${scrolled ? "bg-sand/95 backdrop-blur-md shadow-[0_2px_20px_rgba(116,68,54,0.08)] border-b border-brand/15" : "bg-sand/80 backdrop-blur-sm border-b border-brand/10"}`}>
      <div className="flex justify-between items-center py-2 pr-4 pl-2 md:pl-4 md:pr-6">
        <a href={link("#hero")} onClick={() => setOpen(false)} className="flex items-center" aria-label="Истова — главная">
          <img src="/brand/decor/cloud.webp" alt="Истова" fetchPriority="high" decoding="async" className={`w-auto transition-[background-color,box-shadow,border-color,height,max-height,opacity] duration-300 ease-out ${scrolled ? "h-11 md:h-12" : "h-14 md:h-16"}`} width={512} height={512} />
        </a>
